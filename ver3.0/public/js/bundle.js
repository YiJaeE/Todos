...
/***/ (function(module, exports, __webpack_require__) {
  ...
  function getTodos() {
    fetch('/todos') // response 객체 내에서 todos 배열만 가져와서 렌더하기
    .then(function (res) {
      return res.json();
  ...
  $inputTodo.onkeyup = function (e) {
    ...
    addTodo(content); // 나중에 추가한 todo가 가장 위에 오도록
  
    $inputTodo.value = ''; // todos = [{ id: generateId(), content, completed: false }, ...todos];
    // render();
  ...
  $nav.onclick = function (e) {
    ...
    e.target.classList.add('active'); // 각 카테고리 id 값 할당
  ...
  $todos.onclick = function (e) {
    if (!e.target.classList.contains('remove-todo')) return; // todos = todos.filter(todo => todo.id !== +e.target.parentNode.id);
